...
const PieChartCanvas = forwardRef((props, ref) => {
  ...
  var canvasYSize = Number(
    props.canvasSize + Number(props.titleText.size) * 0.3
  );
  ...
  const draw = (ctx) => {
    ...
    ctx.fillText(
      props.title,
      xPadding + (pieChartXSize - ctx.measureText(props.title).width) / 2,
      Number(props.titleText.size)
    );

    props.fieldsObejct.forEach((element) => {
      if (props.boolCustomColors) ctx.fillStyle = element.pieCustomColor;
      else ctx.fillStyle = element.pieColor;
      ...
      var textXInside =
        pieChartCenterX + pieChartRadius * 0.8 * Math.cos(midAngle);
      var textYInside =
        pieChartCenterY + pieChartRadius * 0.8 * Math.sin(midAngle);

      var textXOutside =
        pieChartCenterX + pieChartRadius * 1.25 * Math.cos(midAngle);
      var textYOutside =
        pieChartCenterY + pieChartRadius * 1.15 * Math.sin(midAngle);
      ...
      var pieValueText;
      if (props.boolDisplayValue) pieValueText = element.pieValue;
      ...
      if (!props.boolLegend) {
        ...
        ctx.fillRect(
          xPadding + pieChartXSize + canvasXSize * 0.02,
          yPadding + element.index * 30,
          cubeSize,
          cubeSize
        );
        ...
        ctx.fillText(
          element.pieName,
          xPadding + pieChartXSize + cubeSize * 1.25 + canvasXSize * 0.02,
          yPadding + element.index * 30 + Number(props.namesText.size)
        );
      }
