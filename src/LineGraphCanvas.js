...
const LineGraphCanvas = forwardRef((props, ref) => {
  const draw = (ctx) => {
    ...
    ctx.fillText(
      props.title,
      paddingX + (canvasX - ctx.measureText(props.title).width) / 2,
      Number(props.titleText.size)
    );
    ...
    props.xValues.forEach((element, index) => {
      ctx.fillText(
        element,
        paddingX + xJump * (index + 0.5) - ctx.measureText(element).width / 2,
        paddingY +
          canvasY +
          Number(dimy - paddingY - canvasY - props.xValuesText.size) / 2 +
          Number(props.xValuesText.size)
      );
    ...
    for (var i = 0; i <= scales; i++) {
      ...
      ctx.fillText(
        ...
        5 + paddingY + canvasY - (canvasY * i) / scales
      );
      ...
      if (props.boolDisplayXLines) {
        ...
        ctx.lineTo(
          paddingX + canvasX,
          paddingY + canvasY - (canvasY * i) / scales
        );
        ctx.stroke();
      }
    }
    ...
    props.fieldsObejct.forEach((array) => {
      array.lineGraphYValues.forEach((element, index) => {
        ...
        else ctx.strokeStyle = array.lineGraphColor;
        ctx.beginPath();
        ctx.moveTo(
          paddingX + xJump * (index + 0.5),
          paddingY +
            turnValueToYPixels(element, newMax - minValue, canvasY, minValue)
        );
        if (index !== props.xValues.length - 1) {
          element2 = array.lineGraphYValues[index + 1];
          ctx.lineTo(
            paddingX + xJump * (index + 1.5),
            paddingY +
              turnValueToYPixels(element2, newMax - minValue, canvasY, minValue)
          );
        }
        if (props.boolCustomColors) ctx.fillStyle = array.lineGraphCustomColor;
        else ctx.fillStyle = array.lineGraphColor;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(
          paddingX + xJump * (index + 0.5),
      ...
      if (props.fieldsObejct.length > 1) {
        ...
        ctx.fillText(
          array.lineGraphName,
          paddingX + canvasX + cubeSize + 10,
          paddingY + (array.index - 1) * 30 + cubeSize
        );
      }
    });
    if (props.boolDisplayNames) {
      ...
      ctx.fillText(
        ...
        (paddingY + Number(props.scalesText.size)) / 2
      );
    }
